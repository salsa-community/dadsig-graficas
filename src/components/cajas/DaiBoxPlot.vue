    ...
    <div class="contenedor-tooltip-svg">
      ...
      <div class="rotation-wrapper-outer">
        <div class="rotation-wrapper-inner">
          <div class="element-to-rotate" :style="{width: `${alto_vis - margen.arriba - margen.abajo}px`, transform: `rotate(-90deg)translateX(calc(-100% - ${.5 * margen.arriba}px))`}">
      ...
      <div class="eje-x">
        <p  v-html="titulo_eje_x" :style="{padding: `${margen.abajo + 10}px ${margen.derecha}px 0 ${margen.izquierda}px `}"></p>
      </div>
    </div>
    <div class="pie">
      <slot name="pie"></slot>
...
<script>
	...
	export default {
		...
		props: {
			...
			margen: {
				type: Object,
				default: function(){return {arriba:30,abajo:40,izquierda:40,derecha:20}}
			},
			alto_vis: {
				type: Number,
			...
			configurandoDimensionesParaCaja() {
				...
				this.data_agrupada = d3.rollup(
					...
					(d)=> {  
						...
						return({q1: q1, median: median, q3: q3, interQuantileRange: interQuantileRange, min: min, max: max, puntos: puntos, promedio: promedio})
					},
